...
const CommunityPage = ({ onAuthRequest, activateCommunity = true }: CommunityPageProps) => {
  ...
  const [shutterState, setShutterState] = useState<'closing' | 'closed' | 'opening' | 'done'>('closing');
  ...
  useEffect(() => {
    ...
    const playSound = () => {
      const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2849/2849-preview.mp3');
      audio.volume = 0.6;
    ...
    const t1 = setTimeout(() => {
      ...
      setRenderReady(true); // Allow heavy rendering once closed
    }, 1000); // Time taken to close (1.0s)
  ...
  useEffect(() => {
    ...
    if (renderReady && !isLoading) {
      // Delay before starting opening animation to ensure paints
      t2 = setTimeout(() => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        snapshot.forEach((doc) => {
          msgs.push({ id: doc.id, ...doc.data({ serverTimestamps: 'estimate' }) } as Post);
        });
        setPosts(msgs);
  ...
  useEffect(() => {
    ...
    if (posts.length > 0) {
      const timeout = setTimeout(fetchRanks, 0); // Immediate rank fetching 
      ...
      <AnimatePresence>
        {shutterState !== 'done' && (
          <div className="fixed inset-0 z-[8000] pointer-events-none flex flex-col">
            ...
            <motion.div
              initial={{ y: "-100%" }}
              animate={{ y: shutterState === 'closing' || shutterState === 'closed' ? "0%" : "-100%" }}
              transition={{ duration: 1.0, ease: [0.65, 0, 0.35, 1] }}
              style={{ willChange: "transform" }}
              className="flex-1 bg-[#3d2314] flex flex-col justify-end items-center pb-8 border-b-[12px] border-[#1a1a1a] shadow-[0_20px_60px_rgba(0,0,0,0.9)] relative overflow-hidden"
            >
              {/* Rust overlay */}
              <div className="absolute inset-0 opacity-40 bg-[radial-gradient(ellipse_at_center,_transparent_0%,_#1a1a1a_100%)]"></div>
              
              {/* Thick Metal Ribs */}
              <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 48px, #000 48px, #000 52px)' }}></div>
              
              {/* Warning Lights */}
              <div className="absolute top-10 w-full flex justify-around px-20 relative z-10">
                {[1, 2, 3].map(i => (
                  <div key={i} className="w-16 h-4 bg-red-600 border border-black shadow-[0_0_15px_rgba(220,38,38,0.8)] rounded-full animate-pulse"></div>
                ))}
              </div>

              {/* Blood splash (simple shapes) */}
              <div className="absolute bottom-10 right-20 w-40 h-16 bg-[#7f1d1d] rounded-full transform rotate-12 opacity-90 border border-[#450a0a]"></div>
              <div className="absolute bottom-4 left-32 w-24 h-12 bg-[#7f1d1d] rounded-full transform -rotate-12 opacity-90 border border-[#450a0a]"></div>

              {/* Caution Tape */}
              <div className="absolute bottom-0 w-[150%] h-14 bg-yellow-400 translate-y-6 rotate-[1.5deg] flex items-center justify-around border-y-8 border-black opacity-95 z-20 skew-x-12">
                 {Array.from({length: 12}).map((_, i) => (
                    <span key={i} className="text-black font-black text-2xl tracking-[0.2em] uppercase">BIOHAZARD <span className="opacity-50">///</span> </span>
                 ))}
              </div>
              <div className="absolute bottom-0 w-[150%] h-14 bg-yellow-500 translate-y-6 rotate-[-1.5deg] flex items-center justify-around border-y-8 border-black opacity-95 z-10 -skew-x-12">
                 {Array.from({length: 12}).map((_, i) => (
                    <span key={i} className="text-black font-black text-2xl tracking-[0.2em] uppercase">BIOHAZARD <span className="opacity-50">///</span> </span>
                 ))}
              </div>
              
              <div className="text-red-500 font-extrabold font-mono text-6xl md:text-9xl tracking-[0.3em] uppercase mb-16 relative z-30 transform translate-y-16" style={{ textShadow: '4px 4px 0px #450a0a, -2px -2px 0px #000' }}>JEE</div>
            </motion.div>
            {/* Bottom Half */}
            <motion.div
              initial={{ y: "100%" }}
              ...
              transition={{ duration: 1.0, ease: [0.65, 0, 0.35, 1] }}
              style={{ willChange: "transform" }}
              className="flex-1 bg-[#3d2314] flex flex-col justify-start items-center pt-8 border-t-[16px] border-[#1a1a1a] shadow-[0_-20px_60px_rgba(0,0,0,0.9)] relative overflow-hidden"
            >
              {/* Rust overlay */}
              <div className="absolute inset-0 opacity-40 bg-[radial-gradient(ellipse_at_center,_transparent_0%,_#1a1a1a_100%)]"></div>
              ...
              {/* Blood drips top edge - no heavy blurs, just borders and rounded divs */}
              <div className="absolute top-0 w-full h-8 bg-[#7f1d1d] border-b-4 border-[#450a0a]"></div>
              <div className="absolute top-6 left-20 w-8 h-24 bg-[#7f1d1d] border-b border-[#450a0a] rounded-b-full"></div>
              <div className="absolute top-6 left-32 w-5 h-12 bg-[#7f1d1d] border-b border-[#450a0a] rounded-b-full"></div>
              <div className="absolute top-6 right-40 w-10 h-32 bg-[#7f1d1d] border-b border-[#450a0a] rounded-b-full"></div>
              <div className="absolute top-6 right-24 w-12 h-16 bg-[#7f1d1d] border-b border-[#450a0a] rounded-b-full"></div>

              <div className="text-red-500 font-extrabold font-mono text-6xl md:text-9xl tracking-[0.3em] uppercase mt-16 relative z-10 transform -translate-y-24" style={{ textShadow: '4px 4px 0px #450a0a, -2px -2px 0px #000' }}>PULSE</div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
