    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-xl max-h-[85vh] bg-[#0a0a0b] border border-white/10 rounded-3xl overflow-hidden flex flex-col shadow-2xl relative"
        >
          {/* Header */}
          <div className="p-4 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-purple-900/20 to-blue-900/20 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-white tracking-tight uppercase">Deep Analysis Engine</h2>
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all self-end sm:self-auto"
            >
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <div className="text-center max-w-md mx-auto mb-6">
              <h3 className="text-xl font-black text-white mb-2">Stop guessing. Start strategizing.</h3>
              <p className="text-[11px] text-white/60 leading-relaxed font-medium px-2">
            ...
            <div className="flex flex-col gap-2.5 max-w-md mx-auto">
              {features.map((feature, i) => (
                <div key={i} className={`p-3 rounded-xl border ${feature.color} flex items-start gap-3 hover:-translate-y-0.5 transition-transform duration-300`}>
                  <div className="p-1.5 bg-white/5 rounded-lg shrink-0">
                    {React.cloneElement(feature.icon, { className: feature.icon.props.className.replace('w-5 h-5', 'w-4 h-4') })}
                  </div>
                  <div>
                    <h4 className="text-[13px] font-black text-white">{feature.title}</h4>
                    <p className="text-[10px] text-white/50 mt-1 font-medium leading-relaxed">{feature.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Footer CTA */}
          <div className="p-4 border-t border-white/10 bg-black/50 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2.5 w-full sm:w-auto">
              <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />
              <p className="text-[10px] text-white/50 font-medium leading-tight">
                Secure one-time unlock. <br className="hidden sm:block" /> Lifetime access to insights.
              </p>
            </div>
            <button
              onClick={onPurchase}
              className="w-full sm:w-auto px-5 py-2.5 rounded-lg font-black uppercase tracking-widest text-[11px] transition-all text-white bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:-translate-y-0.5 flex items-center justify-center gap-2"
            >
