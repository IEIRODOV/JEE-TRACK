    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-4xl max-h-[90vh] bg-[#0a0a0b] border border-white/10 rounded-[32px] overflow-hidden flex flex-col shadow-2xl relative"
        >
          {/* Header */}
          <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-purple-900/20 to-blue-900/20 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400 uppercase tracking-widest">Premium Feature Preview</span>
                </div>
              </div>
            </div>
            <button
              onClick={onClose}
              className="p-3 rounded-xl bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all self-end sm:self-auto"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
            <div className="text-center max-w-2xl mx-auto mb-10">
              <h3 className="text-3xl font-black text-white mb-4">Stop guessing. Start strategizing.</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
              {features.map((feature, i) => (
                <div key={i} className={`p-5 rounded-2xl border ${feature.color} flex items-start gap-4 hover:-translate-y-1 transition-transform duration-300`}>
                  <div className="mt-1 p-2 bg-white/5 rounded-xl shrink-0">
                    {feature.icon}
                  </div>
                  <div>
                    <h4 className="text-base font-black text-white">{feature.title}</h4>
                    <p className="text-xs text-white/50 mt-2 font-medium leading-relaxed">{feature.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Footer CTA */}
          <div className="p-6 border-t border-white/10 bg-black/50 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-6">
